...
for i in `seq 1 $b`; do
  ...
  cat hQ.json | head -n$((2*$i*$l)) | tail -n$((2*$l)) | \
    curl -s -XPOST localhost:9200/hq/verse/_bulk --data-binary @- --output /dev/null; sleep 5;
  curl -s -XGET localhost:9200/hq/_stats/docs;
  node ar_scripts/persian/quran_ar_persian.js $((($i-1)*$l+$s)) $l > ar_scripts/persian/outputs/verses_$(($i*$l)).txt;

  cat ar_scripts/persian/original/tokenized.txt | head -n$(($i*$l)) | tail -n$(($l)) \
        > ar_scripts/persian/original/tokenized_$(($i*$l)).txt

  ./ar_scripts/persian/outputs/compare.sh $(($i*$l))
done
